...
<p class="note">The [response types][response type] are defined by the Fetch
specification [[!FETCH]], and refer to the following:

- `basic` is a same-origin response, and thus the requestor has full access
to read the content.
- `cors` is a valid response to a CORS-enabled request, and thus again the
requestor has full access to read the content.
- `default` is a valid response that is generated by a Service Worker as a
response to the request, so it too is fully readable by the requestor.</p>
